...
const engineers = {
    names: ['Den','Matt','Steve'],
    moneys:[1000, 5000, 2000]

}

for(let i = 0; i < engineers.names.length; i++){

console.log(`Заработная плата ${engineers.names[i]} составляет ${engineers.moneys[i]} рублей `);
...
for(let i = 1; i < numbers3.length; i +=2){
...
console.log(numberSum(numbers6));

//задание 6.2
const numberSumEven = (arrayEven) => {
  let sumEven = 0;
...
console.log(numberSumEven(numbers6));

//задание 6.3,6.4
const numberMax = (arrayMax) => {
  let max = arrayMax[0];
  let indexMax = [];
  ...
  console.log(`максимальное число:${max},индекс элементов массива:${indexMax}`);
  return [max, indexMax];
};
...
let numbers8 = [100,50,7,10,12,16,32,44,443,48,31,32,64,53,22];
const numberNum =  (array, num) =>{
    let arrayNum = [];
    for(let i = 0; i < array.length; i++){
        if(array[i] >= num){
...
console.log(numberNum(numbers8,100));

// Задание 9

// Существует массив польщователей, например:
...
for (let i = 0; i < vegetables.length; i++) {
  let object = { word: vegetables[i], length: vegetables[i].length };
  produce.push(object);
}
console.log(produce);

for(let i = 0; i < produce.length;i++){
   console.log(`${produce[i].word} - ${produce[i].length}`); 
}
